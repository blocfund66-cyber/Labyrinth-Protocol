...
const MinotorusBot = ({ isDarkMode = true, onTriggerMix = null, onNavigateTab = null }) => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-center pointer-events-none">
      ...
      {/* 💬 FLOATING CHAT BOX (MATHEMATICALLY CENTERED ON THE EXACT SAME VERTICAL AXIS AS THE BUTTON) */}
      {isOpen && (
        <div 
          className={`pointer-events-auto w-[calc(100vw-3rem)] sm:w-[410px] h-[560px] max-h-[84vh] mb-3 rounded-2xl shadow-2xl border flex flex-col overflow-hidden animate-fadeIn transition-colors ${
            isDarkMode 
